...
$config['dbmaster'] = array(
                array('127.0.0.1', 'root', 'qy22players', ''),
                );

$db = new DB($config['dbmaster']);
...
class DB{
	 ...
	 * @author 刘紫华 2010-8-3
	 * @param string $tbl 表名
	 * @param array $arrData 需要更新的数据
	 * @param array $where 更新条件
	 * @return 影响行数
	 */
	public function updateTable($tbl, $arrData,$where){
	...
	public function deleteTable($tbl,$where){
		...
		oo::oerror()->writeErrorLog('deleteTable第二个参数$where 错误.');
		die('error');
	}
	/**
	 * 缓存多行数据
	...
	/**
	 * 获得删除表的sql
	 * @author 刘紫华 2011-08-06
	 * @param unknown_type $tbl
	 * @param unknown_type $where
	 */
	public function getDeleteTableSql($tbl,$where){
		$sql = "delete from $tbl where 1";
		if(is_array($where) && count($where)){	
			foreach ($where as $key => $value){
				$value = $this->escape($value);
	 ...
	 * @return 影响行数
	 */
	public function getUpdateTableSql($tbl, $arrData,$where){
		$sql = "update ignore $tbl SET ";
		foreach ((array)$arrData as $key => $value){
			$sql .= "`$key`='" . $this->escape($value) . "',";
		}
	 ...
	 * @param string 
	 */
	public function GetInsert($tbl, $arrData){
		$sql = "INSERT IGNORE INTO $tbl SET ";
		foreach ((array)$arrData as $key => $value){
			$sql .= "`$key`='" . $this->escape($value) . "',";
		}
