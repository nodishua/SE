SkillServerConfig[25073]=
{
    id = 25073;
    name = "净灵-红血将-黑洞（百分比伤害）";
    image = "";
    --攻击位移
  ...
  -------------------------------------------------------------------------------------
    appears= --释放表现
	{
		...
		};
		{
			...
			npctype=8003001;	--npctype和npcname如果配了的话，就走半身像飞入的说话方式，如果没配的话就走头上冒泡泡的方式
			npcname=ds(230215);
			dialog=ds(230137);
			offsettime=0.5;
			lifetime=3;
		...
		[1]={			--警戒弹道
            ...
            bullets=
            {
                [1]=
                {
                    ...
                    moveTime = 4; --弹道运行时间 (追踪弹道情况下，若movetime配了，而movedistance=0，则为时间弹道，时间到了之后击中目标；若movetime和movedistance都配了则会计算一个速度作为弹道的移动速度)
                    moveDistance = 0.0001; --弹道运行长度 如果是0 表示在原点不动
                    singleTargetHitMax = 5; --这个弹道能对单个单位造成多少次伤害
					singleTargetHitInterval = 0.8; --单个目标被击间隔;
...
thisconfig=SkillServerConfig[25073];