	...
	hits={
		...
		[2]={			--伤害弹道，预警特效出现之后，等下出黑暗手和伤害
            ...
            bullets=
            {
                [1]=
                {
                    ...
                    moveRotation = 0; --移动方向 0表示 施法者的向量 其他表示这个向量旋转的角度，   顺时针正数 逆时针负数
                    moveTime = 0.4; --弹道运行时间
                    moveDistance = 7.8; --弹道运行长度 如果是0 表示在原点不动
                    singleTargetHitMax = 1; --这个弹道能对单个单位造成多少次伤害
                    shiftmove = 0;--被击位移距离
					shifttime = 0;--位移时间 --客户端用
                    shiftrotation = 0; --位移默认方向是从施法者到被击者连线方向 加个旋转参数，角度、顺时针正数，逆时针负数			
					affect_ID = 210901;	--弹道效果
					movelimit =  0.8, --行动硬直
					attacklimit = 0.8,--攻击硬直
					mapEffect =
						{
		...
		[3]={			--伤害弹道，预警特效出现之后，等下出黑暗手和伤害
            ...
            bullets=
            {
                ...
                {
                    effectid = 0; --特效id 客户端专用
                    originDistance = 8.3; --弹道原点离释放者多远 近战一般都是0
					originRotation = 0; --弹道原点与施法者面向的角度，顺时针正数，逆时针负数
                    regionType = 2; --1扇形 2圆形 3矩形
                    radius = 2; --半径 圆形扇形区域用到的
                    angle = 0; --扇形的角 扇形专用
...
thisconfig=SkillServerConfig[21090];