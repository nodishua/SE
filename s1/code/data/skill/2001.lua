SkillServerConfig[2001]=
{
	...
	lockDistance = 9;	--主角技能需要，锁定目标需要的范围
    ...
    OnGetShiftPos = function()
        ...
        if currTarget then
            ...
            elseif SkillButtonPanel.GetPlayerFirst() then
                ...
                local player = KingOfFighters.ChooseNearstEnemy(6, LOCAL_CHAR:GetTransform().forward, 360, true);
                if player then
            ...
            local target = KingOfFighters.ChooseNearstEnemy(6, LOCAL_CHAR:GetTransform().forward, 360, false);
            if target then
...
thisconfig=SkillServerConfig[2001];