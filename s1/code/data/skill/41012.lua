...
SkillServerConfig[41012]=
{
	...
	movelimit =  1.6, --行动硬直
	attacklimit = 1.7,--攻击硬直
	svrmovelimit = 1.6,  --服务器用到的行动硬直
	svrattacklimit = 1.7,  --服务器用到的行动硬直
	baseSpeed = 1.7;
	...
	appears= --释放表现
	{
		...
		};]]
		{
			...
			npctype=8002501;	--npctype和npcname如果配了的话，就走半身像飞入的说话方式，如果没配的话就走头上冒泡泡的方式
			npcname=ds(230226);
			dialog=ds(230166);
			offsettime=0;
			lifetime=2;
			...
			bullets=
			{
				[1]=
				{
					...
					mapEffect =				--预警配置
					{
						...
						scale = Vector3.New(3,1,8.5);		--缩放比例，扇形和圆形直接配缩放的值大小，矩形需要配=Vector3.New(x,1,z)x是横向，z是竖向，y不用管
...
thisconfig=SkillServerConfig[41012];