SkillServerConfig[21212]=   --技能ID
{
    id = 21212; --技能ID
    name = "魔将-凝秋-凝血光束"; -- 技能名称
    image = ""; -- 技能图标
    ...
    appears = --释放表现
	{
		...
		
		{
			type = FcAction_Animation; --动作
			offsettime=0.8;
			lifetime=8;	--若lifetime短于真实动作时间，则动作会被截断
			animation="skill02";
			baseSpeed = 1;	--暂时只关联了动作速率,以后关联攻速, 在动作的lifeTime结束的时候,才会还原到1
		};

		{
			...
			npctype=8003017;	--npctype和npcname如果配了的话，就走半身像飞入的说话方式，如果没配的话就走头上冒泡泡的方式
			npcname=ds(230217);
			dialog=ds(230140);
			offsettime=0.5;
			lifetime=5;
		...
		[1]={
            ...
            bullets=
            {
                [1]=
                {
                    ...
                    bulletType = 1; --弹道类型（0直线弹道（默认值），1追踪弹道，2追踪群攻弹道（目标位置为原点），3弧形弹道，4固定弹道，用于一些服务器触发的技能，5连线弹道类型弹道）
					...
					mapEffect =
					{
						...
						id = 20154;
						scale = 2.0;	----缩放比例
						startName = "hud";  --如果是从某个位置飞出来的弹道，可以弹道特效出发点
...
thisconfig=SkillServerConfig[21212];