...
SkillServerConfig[43011]=
{
	...
	baseSpeed = 1.5;	--动作播放速度缩放
    movelimit =  1.5, --行动硬直
    attacklimit = 1.8,--攻击硬直
	svrmovelimit = 1.5,  --服务器用到的行动硬直
	svrattacklimit = 1.8,  --服务器用到的行动硬直
	skillDirInfo = 
	{
  ...
  --------------------------------------------------------------------------------------
    appears= --释放表现
	{
		...
		};
		{
			...
			npctype=8002561;	--npctype和npcname如果配了的话，就走半身像飞入的说话方式，如果没配的话就走头上冒泡泡的方式
			npcname=ds(230245);
			dialog=ds(230188);
			offsettime=0;
			lifetime=2;
			...
			bullets=
			{
				[1]=
				{
					...
					mapEffect =				--预警配置
					{
						...
						scale = Vector3.New(4,1,8.2);		--缩放比例，扇形和圆形直接配缩放的值大小，矩形需要配=Vector3.New(x,1,z)x是横向，z是竖向，y不用管
		...
		[2]=
		{
            ...
            bullets=
            {
                [1]=
                {
                    ...
                    moveRotation = 0; --移动方向 0表示 施法者的向量 其他表示这个向量旋转的角度，   顺时针正数 逆时针负数
                    moveTime = 0.4; --弹道运行时间
                    moveDistance = 8.2; --弹道运行长度 如果是0 表示在原点不动
                    singleTargetHitMax = 1; --这个弹道能对单个单位造成多少次伤害
                    nokill = false ; --不击杀怪物
...
thisconfig=SkillServerConfig[43011];