		...
		[1]={			--警戒弹道
			...
			bullets=
			{
				[1]=
				{
					...
					radius = 1; --半径 圆形扇形区域用到的，矩形的话就是长度
					angle = 0; --扇形的角 扇形专用
					...
					mapEffect =
					{
						...
						scale = Vector3.New(3,1,12);		--缩放比例，扇形和圆形直接配缩放的值大小，矩形需要配=Vector3.New(x,1,z)x是横向，z是竖向，y不用管
		...
		[3]={			--伤害弹道
			...
			bullets=
			{
				[1]=
				{
					...
					originRotation = 0; --弹道原点与施法者面向的角度，顺时针正数，逆时针负数
					regionType = 3; --1扇形 2圆形 3矩形
					radius = 11; --半径 圆形扇形区域用到的，矩形的话就是长度
					angle = 0; --扇形的角 扇形专用
					width = 3;--如果是矩形 radius 就是矩形人朝向方向的长度 width就是横向宽度 
					moveRotation = 0; --移动方向 0表示 施法者的向量 其他表示这个向量旋转的角度，   顺时针正数 逆时针负数
					moveTime = 0; --弹道运行时间
...
thisconfig=SkillServerConfig[21237];