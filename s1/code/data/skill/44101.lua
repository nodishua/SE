SkillServerConfig[44101]=
{
	id = 44101;
	name = "真凤血脉";
	image = "";
	--攻击位移
	shiftmove = 7;--被击位移距离 负数就是往后咯
	shifttime = 0.3;--位移时间
	shiftoffset = 6.5; --释放多久后开始位移 --这个服务器忽略可以， 直接一放技能就到目标点
	movelimit =  7.5, --行动硬直
	attacklimit = 7.5,--攻击硬直
	--客户端专用----------------------------------------------------------------------------
	...
	appears= --释放表现
	{
		...
		};
		{
			...
			npctype=8002584;	--npctype和npcname如果配了的话，就走半身像飞入的说话方式，如果没配的话就走头上冒泡泡的方式
			npcname=ds(230267);
			dialog=ds(230213);
			offsettime=0;
			lifetime=2;
	...
	hits=
	{
		...
		{ -- 伤害效果
			...
			bullets=
			{
				...
				{
					effectid = 0; --特效id 客户端专用
					originDistance = 6.5; --弹道原点离释放者多远 近战一般都是0
					originRotation = 0; --弹道原点与施法者面向的角度，顺时针正数，逆时针负数
					regionType =2; --1扇形 2圆形 3矩形
					radius = 8; --半径 圆形扇形区域用到的
					angle = 0; --扇形的角 扇形专用
...
thisconfig=SkillServerConfig[44101];