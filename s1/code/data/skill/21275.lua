	...
	{
		{
			...
			lifetime=1;
			animation="attack03";
			baseSpeed = 2;	--暂时只关联了动作速率,以后关联攻速, 在动作的lifeTime结束的时候,才会还原到1
		};
		{
	    ...
	    [1]=
		    {
			    ...
			    bullets=
		       {
				  [1]=
				{
					...
					angle = 0; --扇形的角 扇形专用
					width = 2;--如果是矩形 radius 就是矩形人朝向方向的长度 width就是横向宽度 
					...
					{
						offset = 0;
						lifetime = 1.00;	--预警的播放时间，要和技能真实出现对应
						id = 9999995;			--预警的特效ID，要配主任特殊处理过的9999991-30度、9999992-45度、9999993-60度、9999994-1米半径的圈形、9999995-1米的矩形预警
						scale = Vector3.New(2,1,6);		--缩放比例，扇形和圆形直接配缩放的值大小，矩形需要配=Vector3.New(x,1,z)x是横向，z是竖向，y不用管
		...
		[3]={			--伤害弹道，预警特效出现之后，等下出黑暗手和伤害
            ...
            bullets=
            {
                [1]=
                {
                    ...
                    radius = 6; --半径 圆形扇形区域用到的
                    angle = 0; --扇形的角 扇形专用
					width = 1.5;--如果是矩形 radius 就是矩形人朝向方向的长度 width就是横向宽度 
                    moveRotation = 0; --移动方向 0表示 施法者的向量 其他表示这个向量旋转的角度，   顺时针正数 逆时针负数
                    moveTime = 0; --弹道运行时间
                    moveDistance = 0; --弹道运行长度 如果是0 表示在原点不动
                    singleTargetHitMax = 1; --这个弹道能对单个单位造成多少次伤害
...
thisconfig=SkillServerConfig[21275];