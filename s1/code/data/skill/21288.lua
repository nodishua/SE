...
SkillServerConfig[21288]=   --技能ID
{
	...
	ifsevermove = false;	--true则位移生效、false则位移不生效shiftrotation = 0; --位移与朝向的夹角0表示往前，正数顺时针，单位是角度数
    movelimit =  6.4, --行动硬直
    attacklimit = 6.4,--攻击硬直
	svrmovelimit = 6.4,  --服务器用到的行动硬直
	svrattacklimit = 6.4,  --服务器用到的行动硬直
	...
	{
		{
			...
			offsettime=0.0;
			lifetime=5.8;	--若lifetime短于真实动作时间，则动作会被截断
			animation="skill01";
		...
		[1]={
            ...
            bullets=
            {
                [1]=
                {
					...
					{
						offset = 0;
						lifetime = 2.7;	--预警的播放时间，要和技能真实出现对应
						id = 9999994;			--预警的特效ID，要配主任特殊处理过的9999991——9999995
						scale = 5;		--缩放比例，扇形和圆形直接配缩放的值大小，矩形需要配=Vector3.New(x,1,z)x是横向，z是竖向，y不用管
						timeScale = 1/2.7;  --预警的播放速率，要是持续时间的倒数
					};
					appears = --这个弹道被打倒的表现
					{
		...
		[2]={
            ...
            bullets=
            {
                [1]=
                {
                    ...
                    radius = 5; --半径 圆形扇形区域用到的
                    angle = 0; --扇形的角 扇形专用
                    width = 0.0;--如果是矩形 radius 就是矩形人朝向方向的长度 width就是横向宽度                   
                    bulletType = 0; --弹道类型（0直线弹道（默认值）,1固定目标类型追踪弹道，2固定目标原点（程序计算）类型追踪弹道，3弧形弹道，4固定原点（根据配置计算）类型弹道，5连线弹道类型弹道）
                    moveRotation = 0; --移动方向 0表示 施法者的向量 其他表示这个向量旋转的角度，   顺时针正数 逆时针负数
                    moveTime = 0; --弹道运行时间 (追踪弹道情况下，若movetime配了，而movedistance=0，则为时间弹道，时间到了之后击中目标；若movetime和movedistance都配了则会计算一个速度作为弹道的移动速度；服务器心跳检测为0.1秒，一般来说怪物的最小被击范围是1米，所以弹道速度最大不能超过10m/s)
                    moveDistance = 0; --弹道运行长度 如果是0 表示在原点不动
...
thisconfig=SkillServerConfig[21288];