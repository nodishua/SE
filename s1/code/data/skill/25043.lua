SkillServerConfig[25043]=
{
	id = 25043;
	name = "净灵-黑衣刺客-刺杀（百分比伤害）";
	image = "";
	--攻击位移
	shiftmove = 8;--被击位移距离 负数就是往后咯
	shifttime = 0.35;--位移时间
	shiftoffset = 1.3; --释放多久后开始位移 --这个服务器忽略可以， 直接一放技能就到目标点
	movelimit =  3.9, --行动硬直
	attacklimit = 3.9,--攻击硬直
	...
	hits={
		...
		[1]={ --预警弹道
			...
			bullets=
			{
				[1]=
				{
					...
					{
						offset = 0;
						lifetime = 1.3;	--预警的播放时间，要和技能真实出现对应
						id = 9999995;			--预警的特效ID，要配主任特殊处理过的9999991——9999995
						scale = Vector3.New(2,1,10);		--缩放比例，扇形和圆形直接配缩放的值大小，矩形需要配=Vector3.New(x,1,z)x是横向，z是竖向，y不用管
						timeScale = 1/1.3;  --预警的播放速率，要是持续时间的倒数
					};
				};
			};
		}; 
		...
		[3]={ --刺客的第一击
			...
			bullets=
			{
				[1]=
				{
					effectid = 0.6; --特效id 客户端专用
					originDistance = -10; --弹道原点离释放者多远 近战一般都是0
					originRotation = 180; --弹道原点与施法者面向的角度，顺时针正数，逆时针负数
					regionType = 3; --1扇形 2圆形 3矩形
					radius = 10.1; --半径 圆形扇形区域用到的
					angle = 0; --扇形的角 扇形专用
					width = 2.0;--如果是矩形 radius 就是矩形人朝向方向的长度 width就是横向宽度  
					moveRotation = 0; --移动方向 0表示 施法者的向量 其他表示这个向量旋转的角度，   顺时针正数 逆时针负数
					moveTime = 0; --弹道运行时间
...
thisconfig=SkillServerConfig[25043];