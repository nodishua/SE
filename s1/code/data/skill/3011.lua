SkillServerConfig[3011]=
{
	Max_Level=75;
    id = 3011;
    name = "焰锋";
    image = "";
	bubbleName = ds(230059);	--战斗冒字技能名称
    --攻击位移
		...
		[2]=
		{
            ...
            bullets=
            {
                [1]=
                {
					hate = 20; --技能仇恨
                    effectid = 0; --特效id 客户端专用
                    originDistance = 1.3; --弹道原点离释放者多远 近战一般都是0
				    originRotation = 0; --弹道原点与施法者面向的角度，顺时针正数，逆时针负数
                    regionType = 2; --1扇形 2圆形 3矩形
                    ...
                    moveDistance = 7.2; --弹道运行长度 如果是0 表示在原点不动
                    singleTargetHitMax = 1; --这个弹道能对单个单位造成多少次伤害
					singleTargetHitInterval = 0.3; --单个目标被击间隔;
                    maxhit = 5; --弹道最多命中的目标数量
					maxrolehit = 1; --弹道最多命中角色的目标数量
...
thisconfig=SkillServerConfig[3011];