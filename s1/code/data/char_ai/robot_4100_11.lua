robot_4100_11=
{
	...
	init=function(char_type, char_key_1, char_key_2, jingjie, lvl)
		...
		--设置职业
		SetPre(char_type, char_key_1, char_key_2, robot_4100_11.PreEnum[preIndex]);
		--设置性别
	...
	timeout=function(char_type, char_key_1, char_key_2, time_sec, time_m_sec)
	  local ai_state = GetAiState(char_type, char_key_1, char_key_2);
		...
		if ai_state == 0 then
			robot_4100_11.ProcessBorn(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 1 then
			robot_4100_11.ProcessStand(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 2 then
			robot_4100_11.ProcessPatrol(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 3 then
			robot_4100_11.ProcessWalk(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 4 then
			robot_4100_11.ProcessRunAway(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 5 then
			robot_4100_11.ProcessGoBack(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 6 then
			robot_4100_11.ProcessFighting(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 7 then
			robot_4100_11.ProcessFlow(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		end;
	end;
	...
	--nil
		...
		local x, y;
		if camp == 1 then
			x = robot_4100_11.LinePos[1][pos_index].pos_x + CRandom(0, 10);
			y = robot_4100_11.LinePos[1][pos_index].pos_y + CRandom(0, 10);
		else
			x = robot_4100_11.LinePos[14][pos_index].pos_x + CRandom(0, 10);
			y = robot_4100_11.LinePos[14][pos_index].pos_y + CRandom(0, 10);
		end;
	...
	ProcessStand=function(char_type, char_key_1, char_key_2, cur_state,camp,pre)
		...
		robot_4100_11.PorcessFindEnemy(char_type, char_key_1, char_key_2, cur_state,camp,pre,30);
	...
	ProcessPatrol=function(char_type, char_key_1, char_key_2, cur_state,camp,pre)
	   ...
	   local str1 = string.format("raymond record tower:%d|%d|%d", a_tw,b_tw,camp);
		...
		if CheckRoleIfInDanger(char_type, char_key_1, char_key_2) == 1 then
			line_index = robot_4100_11.PatrolPos[a_tw..b_tw..camp].NormalLine;
			x = robot_4100_11.LinePos[line_index][pos_index].pos_x + CRandom(0, 10);
			y = robot_4100_11.LinePos[line_index][pos_index].pos_y + CRandom(0, 10);
			RunToPos(char_type, char_key_1, char_key_2,x,y,0);
	  ...
	  --血量小于50%，我就回一波血
		local recover_skill = robot_4100_11.MagicSkill[0];
		...
		if hp_percent < 35 and patrol_action < 80 then
			...
			if CheckCharWalkEnd(char_type, char_key_1, char_key_2) ==1  then	
				...
				if  HpLineRandom < 8 then
					line_index = robot_4100_11.PatrolPos[a_tw..b_tw..camp].RecoverLine;
				elseif HpLineRandom < 9 then
					line_index = robot_4100_11.PatrolPos[a_tw..b_tw..camp].WatchLine;
				else
					line_index = robot_4100_11.PatrolPos[a_tw..b_tw..camp].DefendLine;
				end;
				pos_index=CRandom(0,robot_4100_11.LinePos[line_index].pos_num);
				x = robot_4100_11.LinePos[line_index][pos_index].pos_x + CRandom(0, 10);
				y = robot_4100_11.LinePos[line_index][pos_index].pos_y + CRandom(0, 10);
				RunToPos(char_type, char_key_1, char_key_2,x,y,0);
			end;
		elseif hp_percent < 50 and patrol_action < 20  and robot_4100_11.PreSpecial[pre].IfShortPre == 1
				then  --血量小于50%，游荡于猥琐线和正常线范围
			if  robot_4100_11.PreSpecial[pre].IfShortPre == 1 then
				guard = 60;
			else
				guard = 90;
			end;
		...
		elseif patrol_action < attack_random   then--其他情况，游荡于正常线和攻击线范围
			...
			if CheckCharWalkEnd(char_type, char_key_1, char_key_2) ==1  then	
				if CRandom(0,10) < 2 or robot_4100_11.PreSpecial[pre].IfShortPre == 0  then
					line_index = robot_4100_11.PatrolPos[a_tw..b_tw..camp].NormalLine;
				else
					line_index = robot_4100_11.PatrolPos[a_tw..b_tw..camp].AttackLine;
				end;
				pos_index=CRandom(0,robot_4100_11.LinePos[line_index].pos_num);
				x = robot_4100_11.LinePos[line_index][pos_index].pos_x + CRandom(0, 10);
				y = robot_4100_11.LinePos[line_index][pos_index].pos_y + CRandom(0, 10);
		...
		robot_4100_11.PorcessFindEnemy(char_type, char_key_1, char_key_2, cur_state,camp,pre,guard);
	...
	ProcessRunAway=function(char_type, char_key_1, char_key_2, cur_state,camp,pre)
		...
		if CheckCharWalkEnd(char_type, char_key_1, char_key_2) ==1  then
			SetAiState(char_type, char_key_1, char_key_2, 2);
			robot_4100_11.ProcessPatrol(char_type, char_key_1, char_key_2, 2,camp,pre);
			return;
			end;
		Debug(char_type, char_key_1, char_key_2,0, str);
	end;
	...
	ProcessFighting=function(char_type, char_key_1, char_key_2, cur_state,camp,pre)
	 ...
	 --检测当前目标是否已死亡，死亡，则重新寻找目标
		...
		local love_index = char_key_1%robot_4100_11.PreSpecial[pre][3].SkillNum;
		local love_skill = robot_4100_11.PreSpecial[pre][3][love_index];
		...
		for i=0, robot_4100_11.PreSpecial[pre][jingjie].SkillNum do
			if CastSkillCheck(char_type, char_key_1, char_key_2, robot_4100_11.PreSpecial[pre][jingjie][i], 1) == 0 then
			CastSkill(char_type, char_key_1, char_key_2,
			tar_char.char_type, tar_char.character_id_1, tar_char.character_id_2,
			robot_4100_11.PreSpecial[pre][jingjie][i], 1);
			return;
			end;
		end;
