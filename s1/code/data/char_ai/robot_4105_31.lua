robot_4105_31=
{
	...
	robot_task = {
		{
			{0, 2, 0, 1, 0, 0},  --1
			{1, 2, 0, 2552, 2313, 2313},  --2
			{1, 2, 0, 2224, 2505, 2505},  --3
			{1, 2, 0, 2510, 2949, 2949},  --4--重复点
			{1, 2, 0, 2335, 2797, 2797},  --5
			{1, 2, 0, 2447, 2671, 2671},  --6
		},
		{
			{0, 2, 0, 1, 0, 0},  --1
			{1, 2, 0, 2552, 2313, 2313},  --2
			{1, 2, 0, 2876, 2548, 2548},  --3
			{1, 2, 0, 2600, 2861, 2861},  --4--重复点
			{1, 2, 0, 2791, 2798, 2798},  --5
			{1, 2, 0, 2655, 2648, 2648},  --6
		},
		{
			{0, 2, 0, 1, 0, 0},  --1
			{1, 2, 0, 2543, 3428, 3428},  --2
			{1, 2, 0, 2864, 3329, 3329},  --3
			{1, 2, 0, 2606, 2953, 2953},  --4--重复点
			{1, 2, 0, 2792, 2991, 2991},  --5
			{1, 2, 0, 2646, 3132, 3132},  --6
		},
		{
			{0, 2, 0, 1, 0, 0},  --1
			{1, 2, 0, 2543, 3428, 3428},  --2
			{1, 2, 0, 2206, 3303, 3303},  --3
			{1, 2, 0, 2507, 2953, 2953},  --4--重复点
			{1, 2, 0, 2341, 2982, 2982},  --5
			{1, 2, 0, 2443, 3145, 3145},  --6
		},
	};
	...
	init=function(char_type, char_key_1, char_key_2, jingjie, lvl)
		...
		CSleep(char_type, char_key_1, char_key_2, 2);
	end;
	...
	timeout=function(char_type, char_key_1, char_key_2, time_sec, time_m_sec)
	    ...
	    if task_index <= 0 then
		   ...
		   if robot_4105_31.robot_task[own_task_list][task_index][1] == 0 then
		      CSleep(char_type, char_key_1, char_key_2, robot_4105_31.robot_task[own_task_list][task_index][4]);
		      return;
		   end;

		   SetRobotTask(char_type, char_key_1, char_key_2,
		   robot_4105_31.robot_taskown_task_list[own_task_list][task_index][1], 
		   robot_4105_31.robot_task[own_task_list][task_index][4], 
		   robot_4105_31.robot_task[own_task_list][task_index][5],
		   robot_4105_31.robot_task[own_task_list][task_index][6]);
		   SetRobotWalkTarget(char_type, char_key_1, char_key_2);  
	    end;
		
		--是否完成任务
		local if_finish = robot_4105_31.CheckTaskFinished(char_type, char_key_1, char_key_2);
		if if_finish == true then
           if task_index >= 6 then
              task_index = 4;
           else
		   ...
		   SetRobotTask(char_type, char_key_1, char_key_2,
		    robot_4105_31.robot_task[own_task_list][task_index][1], 
		    robot_4105_31.robot_task[own_task_list][task_index][4], 
		   	robot_4105_31.robot_task[own_task_list][task_index][5], 
		   	robot_4105_31.robot_task[own_task_list][task_index][6]);
		   SetRobotWalkTarget(char_type, char_key_1, char_key_2); 
		...
		if ai_state == 0 then
			robot_4105_31.ProcessBorn(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 1 then
			robot_4105_31.ProcessStand(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 2 then
			robot_4105_31.ProcessPatrol(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 3 then
			robot_4105_31.ProcessWalk(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 4 then
			robot_4105_31.ProcessRunAway(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 5 then
			robot_4105_31.ProcessGoBack(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 6 then
			robot_4105_31.ProcessFighting(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 7 then
			robot_4105_31.ProcessFlow(char_type, char_key_1, char_key_2, ai_state);
		end;
	end;
	...
	CheckTaskFinished=function(char_type, char_key_1, char_key_2)
	    --判定任务类型, 做相应的处理
			...
			if robot_4105_31.robot_task[own_task_list][task_index][1] == 0 then
				...
				--休息交给stand状态去处理
			elseif robot_4105_31.robot_task[own_task_list][task_index][1] == 1 then
		    	--检测是否已经到目标点
		    	if CheckDistance(char_type, char_key_1, char_key_2, robot_4105_31.robot_task[own_task_list][task_index][4], robot_4105_31.robot_task[own_task_list][task_index][6], 20) == 1 then
		    		return true;
		    	end;
			elseif 	robot_4105_31.robot_task[own_task_list][task_index][1] == 2 then
				--检测当前任务需要的type数量是否达标
				local kill_npc_num = GetTaskNpcKillNum(char_type, char_key_1, char_key_2);
				if kill_npc_num >= robot_4105_31.robot_task[own_task_list][task_index][5] then
					return true;
				elseif GetNearestTypeNPC(char_type, char_key_1, char_key_2, robot_4105_31.robot_task[own_task_list][task_index][4]).character_id_1 == 0 then
					return true;
				end;	  
			elseif  robot_4105_31.robot_task[own_task_list][task_index][1] == 3 then
				--检测目标血量是否达标
				local tar_hp_percent = GetTaskNpcHpPercent(char_type, char_key_1, char_key_2);
				if tar_hp_percent <= robot_4105_31.robot_task[own_task_list][task_index][5] then
					return true;	
			    elseif GetUnityNPC(char_type, char_key_1, char_key_2, robot_4105_31.robot_task[own_task_list][task_index][4]).character_id_1 == 0 then
					return true;
				end;
			elseif  robot_4105_31.robot_task[own_task_list][task_index][1] == 4 then
				local collect_npc_num = GetTaskNpcNum(char_type, char_key_1, char_key_2);
				if collect_npc_num >= robot_4105_31.robot_task[own_task_list][task_index][5] then
					return true;
				elseif GetNearestTypeNPC(char_type, char_key_1, char_key_2, robot_4105_31.robot_task[own_task_list][task_index][4]).character_id_1 == 0 then
					return true;
				end; 
			elseif robot_4105_31.robot_task[own_task_list][task_index][1] == 5 then
				local collect_npc_num = GetTaskNpcNum(char_type, char_key_1, char_key_2);
				if collect_npc_num >= robot_4105_31.robot_task[own_task_list][task_index][5] then
					return true;
				elseif GetUnityNPC(char_type, char_key_1, char_key_2, robot_4105_31.robot_task[own_task_list][task_index][4]).character_id_1 == 0 then
	...
	ProcessWalk=function(char_type, char_key_1, char_key_2, cur_state)
		...
		if hate_char.character_id_1 ~= 0 and robot_4105_31.robot_task[own_task_list][task_index][2] >= 1 then
		   --你要战，便作战
		...
		if robot_4105_31.robot_task[own_task_list][task_index][2] == 2 then
		   --我就是好战
		...
		if robot_4105_31.robot_task[own_task_list][task_index][1] == 4 or robot_4105_31.robot_task[own_task_list][task_index][1] == 5 then
			check_distance = 2;
		end;

		--如果到达移动目标点
		if CheckDistance(char_type, char_key_1, char_key_2, tar_pos.pos_x, tar_pos.pos_y, check_distance) == 1 then
			if robot_4105_31.robot_task[own_task_list][task_index][1] == 1 then
		    	--进入stand状态
		    	SetAiState(char_type, char_key_1, char_key_2, 1);
		    	return;
			elseif 	robot_4105_31.robot_task[own_task_list][task_index][1] == 2 then
				--找一个当前任务需要的type_npc
				local task_char = GetNearestTypeNPC(char_type, char_key_1, char_key_2, robot_4105_31.robot_task[own_task_list][task_index][4]);
			...
			elseif  robot_4105_31.robot_task[own_task_list][task_index][1] == 3 then
				--检测目标血量是否达标
				local task_char = GetUnityNPC(char_type, char_key_1, char_key_2, robot_4105_31.robot_task[own_task_list][task_index][4]);
				if task_char.character_id_1 == 0 then
				   SetAiState(char_type, char_key_1, char_key_2, 1);
		    	   return;
		    	end;
			...
			elseif  robot_4105_31.robot_task[own_task_list][task_index][1] == 4 then
		    	...
		    	CSleep(char_type, char_key_1, char_key_2, robot_4105_31.robot_task[own_task_list][task_index][6]);
			    SetAiState(char_type, char_key_1, char_key_2, 1);
			    return;
			elseif robot_4105_31.robot_task[own_task_list][task_index][1] == 5 then
				local task_char = GetUnityNPC(char_type, char_key_1, char_key_2, robot_4105_31.robot_task[own_task_list][task_index][4]);
