robot_3017_11=
{
	...
	PreSpecial=
	{
		...
		[31]={--对应MOBA境界
			IfShortPre= 1;attack_range = 25;attack_error=35;
			[0]={SkillNum=1;[0]=3001;};[1]={SkillNum=2;[1]=3001;[0]=3017;};[2]={SkillNum=3;[2]=3001;[1]=3017;[0]=3021;};
			[3]={SkillNum=4;[3]=3001;[2]=3017;[1]=3021;[0]=3031;};
		};
		--弓手
		[41]={--对应MOBA境界
	...
	init=function(char_type, char_key_1, char_key_2, jingjie, lvl)
		...
		--设置职业
		SetPre(char_type, char_key_1, char_key_2, robot_3017_11.PreEnum[preIndex]);
		--设置性别
	...
	timeout=function(char_type, char_key_1, char_key_2, time_sec, time_m_sec)
	  local ai_state = GetAiState(char_type, char_key_1, char_key_2);
		...
		if ai_state == 0 then
			robot_3017_11.ProcessBorn(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 1 then
			robot_3017_11.ProcessStand(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 2 then
			robot_3017_11.ProcessPatrol(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 3 then
			robot_3017_11.ProcessWalk(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 4 then
			robot_3017_11.ProcessRunAway(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 5 then
			robot_3017_11.ProcessGoBack(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 6 then
			robot_3017_11.ProcessFighting(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		elseif ai_state == 7 then
			robot_3017_11.ProcessFlow(char_type, char_key_1, char_key_2, ai_state,camp,pre);
		end;
	end;
	...
	ProcessStand=function(char_type, char_key_1, char_key_2, cur_state,camp,pre)
		...
		robot_3017_11.PorcessFindEnemy(char_type, char_key_1, char_key_2, cur_state,camp,pre,400);
	...
	ProcessPatrol=function(char_type, char_key_1, char_key_2, cur_state,camp,pre)
	   ...
	   local guard = 400;
		...
		if CheckCharWalkEnd(char_type, char_key_1, char_key_2) ==1  then
			...
			local temp_index = 0;
			if relive_x == robot_3017_11.RelivePos[1].x then
				temp_index=1;
			elseif relive_x == robot_3017_11.RelivePos[2].x then
				temp_index=2;
			end;
			local target_x = CRandom(0, robot_3017_11.TargetPos[temp_index].mov_len) 
			+ robot_3017_11.TargetPos[temp_index].x- robot_3017_11.TargetPos[temp_index].mov_short;
			local target_y = CRandom(0,robot_3017_11.TargetPos[temp_index].mov_len) 
			+ robot_3017_11.TargetPos[temp_index].y-robot_3017_11.TargetPos[temp_index].mov_short;
			RunToPos(char_type, char_key_1, char_key_2,target_x,target_y,0);
		end;


	  --血量小于30%，我就回一波血
		local recover_skill = robot_3017_11.MagicSkill[0];
		...
		robot_3017_11.PorcessFindEnemy(char_type, char_key_1, char_key_2, cur_state,camp,pre,guard);
	...
	ProcessRunAway=function(char_type, char_key_1, char_key_2, cur_state,camp,pre)
		...
		if CheckCharWalkEnd(char_type, char_key_1, char_key_2) ==1  then
			SetAiState(char_type, char_key_1, char_key_2, 2);
			robot_3017_11.ProcessPatrol(char_type, char_key_1, char_key_2, 2,camp,pre);
			return;
			end;
		Debug(char_type, char_key_1, char_key_2,0, str);
	end;
	...
	ProcessFighting=function(char_type, char_key_1, char_key_2, cur_state,camp,pre)
	 ...
	 --检测当前目标是否已死亡，死亡，则重新寻找目标
		...
		local love_index = char_key_1%robot_3017_11.PreSpecial[pre][3].SkillNum;
		local love_skill = robot_3017_11.PreSpecial[pre][3][love_index];
		...
		for i=0, robot_3017_11.PreSpecial[pre][jingjie].SkillNum do
			if CastSkillCheck(char_type, char_key_1, char_key_2, robot_3017_11.PreSpecial[pre][jingjie][i], 1) == 0 then
			CastSkill(char_type, char_key_1, char_key_2,
			tar_char.char_type, tar_char.character_id_1, tar_char.character_id_2,
			robot_3017_11.PreSpecial[pre][jingjie][i], 1);
			return;
			end;
		end;
	...
	PorcessFindEnemy=function(char_type, char_key_1, char_key_2, cur_state,camp,pre,guard)
		...
		robot_3017_11.ProcessFighting(char_type, char_key_1, char_key_2, 6,camp,pre);
	-- body
	end;
