robot_4203_11=
{
	...
	--{6, 2, 0, x, n, 0}跟随type_id=x的NPC, 途中发现敌人即攻击
	robot_task = {
	{0, 0, 0, 10, 0, 0},  --process 1
	{1, 2, 0, 2804, 2262, 2262},   --2 p2 走到坐标(x, y, z)点，途中主动发现并攻击敌人 第一波小怪点
	{1, 2, 0, 3268, 2261, 2261},   --3 p3 走到坐标(x, y, z)点，途中主动发现并攻击敌人 第二波小怪
	{1, 2, 0, 3362, 2209, 2209},   --4 p4 走到坐标(x, y, z)点，途中主动发现并攻击敌人 1号BOSS入口点
	{1, 2, 0, 3660, 2196, 2196},   --5 p5 走到坐标(x, y, z)点，途中主动发现并攻击敌人 1号BOSS
	{1, 2, 0, 3654, 1679, 1679},   --6 p5 走到坐标(x, y, z)点，途中主动发现并攻击敌人 2号BOSS入口点
	{1, 2, 0, 3648, 1308, 1308},   --7 p6 走到坐标(x, y, z)点，途中主动发现并攻击敌人 2号BOSS点
	{1, 2, 0, 3282, 1211, 1211},   --8 p7 走到坐标(x, y, z)点，途中主动发现并攻击敌人 3号BOSS入口点
	{1, 2, 0, 2870, 1220, 1220},   --9 p7 走到坐标(x, y, z)点，途中主动发现并攻击敌人 3号BOSS点
	{6, 2, 0, 420300019, 0, 0},			   --跟随unity_id=x的NPC, 途中发现敌人即攻击
	{6, 2, 0, 420300019, 0, 0},			   --跟随unity_id=x的NPC, 途中发现敌人即攻击
	{6, 2, 0, 420300019, 0, 0},			   --跟随unity_id=x的NPC, 途中发现敌人即攻击
	{6, 2, 0, 420300019, 0, 0},			   --跟随unity_id=x的NPC, 途中发现敌人即攻击
	{6, 2, 0, 420300019, 0, 0},			   --跟随unity_id=x的NPC, 途中发现敌人即攻击
	...
	init=function(char_type, char_key_1, char_key_2, jingjie, lvl)
		...
		SetPos(char_type, char_key_1, char_key_2, robot_4203_11.born_x, robot_4203_11.born_y, 0);
	...
	timeout=function(char_type, char_key_1, char_key_2, time_sec, time_m_sec)
	    ...
	    --没有任务，或者副本进度已经发生变化
	    robot_4203_11.ReAollocTask(char_type, char_key_1, char_key_2);

        --机器人状态部分额 0出生 1待机 2巡逻 3寻路 4逃跑 5回位 6战斗 7跟随
	    local ai_state = GetAiState(char_type, char_key_1, char_key_2);
		if ai_state == 0 then
			robot_4203_11.ProcessBorn(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 1 then
			robot_4203_11.ProcessStand(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 2 then
			robot_4203_11.ProcessPatrol(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 3 then
			robot_4203_11.ProcessWalk(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 4 then
			robot_4203_11.ProcessRunAway(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 5 then
			robot_4203_11.ProcessGoBack(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 6 then
			robot_4203_11.ProcessFighting(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 7 then
			robot_4203_11.ProcessFlow(char_type, char_key_1, char_key_2, ai_state);
		end;
	end;
	...
	CheckTaskFinished=function(char_type, char_key_1, char_key_2)
	    --判定任务类型, 做相应的处理
			...
			if robot_4203_11.robot_task[task_index][1] == 0 then
				if CheckSleepEnd(char_type, char_key_1, char_key_2) <= 0 then
					return true;
				end;
				--休息交给stand状态去处理
			elseif robot_4203_11.robot_task[task_index][1] == 1 then
		    	--检测是否已经到目标点
		    	if CheckDistance(char_type, char_key_1, char_key_2, robot_4203_11.robot_task[task_index][4], robot_4203_11.robot_task[task_index][6], 20) == 1 then
		    		return true;
		    	end;
			elseif 	robot_4203_11.robot_task[task_index][1] == 2 then
				--检测当前任务需要的type数量是否达标
				local kill_npc_num = GetTaskNpcKillNum(char_type, char_key_1, char_key_2);
				if kill_npc_num >= robot_4203_11.robot_task[task_index][5] then
					return true;
				elseif GetNearestTypeNPC(char_type, char_key_1, char_key_2, robot_4203_11.robot_task[task_index][4]).character_id_1 == 0 then
					return true;
				end;	  
			elseif  robot_4203_11.robot_task[task_index][1] == 3 then
				--检测目标血量是否达标
				local tar_hp_percent = GetTaskNpcHpPercent(char_type, char_key_1, char_key_2);
				if tar_hp_percent <= robot_4203_11.robot_task[task_index][5] then
					return true;	
			    elseif GetUnityNPC(char_type, char_key_1, char_key_2, robot_4203_11.robot_task[task_index][4]).character_id_1 == 0 then
					return true;
				end;
			elseif  robot_4203_11.robot_task[task_index][1] == 4 then
				local collect_npc_num = GetTaskNpcNum(char_type, char_key_1, char_key_2);
				if collect_npc_num >= robot_4203_11.robot_task[task_index][5] then
					return true;
				elseif GetNearestTypeNPC(char_type, char_key_1, char_key_2, robot_4203_11.robot_task[task_index][4]).character_id_1 == 0 then
					return true;
				end; 
			elseif robot_4203_11.robot_task[task_index][1] == 5 then
				local collect_npc_num = GetTaskNpcNum(char_type, char_key_1, char_key_2);
				if collect_npc_num >= robot_4203_11.robot_task[task_index][5] then
					return true;
				elseif GetUnityNPC(char_type, char_key_1, char_key_2, robot_4203_11.robot_task[task_index][4]).character_id_1 == 0 then
			...
			elseif robot_4203_11.robot_task[task_index][1] == 6 then
				if  GetUnityNPC(char_type, char_key_1, char_key_2, robot_4203_11.robot_task[task_index][4]).character_id_1 == 0 then
					return true;
				end; 
			end;
	...
	ProcessWalk=function(char_type, char_key_1, char_key_2, cur_state)
		...
		if hate_char.character_id_1 ~= 0 and robot_4203_11.robot_task[task_index][2] >= 1 then
		   --你要战，便作战
		...
		if robot_4203_11.robot_task[task_index][2] == 2 then
		   --我就是好战
		...
		if robot_4203_11.robot_task[task_index][1] == 4 or robot_4203_11.robot_task[task_index][1] == 5 then
			check_distance = 2;
		end;

		--如果到达移动目标点
		if CheckDistance(char_type, char_key_1, char_key_2, tar_pos.pos_x, tar_pos.pos_y, check_distance) == 1 then
			if robot_4203_11.robot_task[task_index][1] == 1 then
		    	--进入stand状态
		    	SetAiState(char_type, char_key_1, char_key_2, 1);
		    	return;
			elseif 	robot_4203_11.robot_task[task_index][1] == 2 then
				--找一个当前任务需要的type_npc
				local task_char = GetNearestTypeNPC(char_type, char_key_1, char_key_2, robot_4203_11.robot_task[task_index][4]);
			...
			elseif  robot_4203_11.robot_task[task_index][1] == 3 then
				--检测目标血量是否达标
				local task_char = GetUnityNPC(char_type, char_key_1, char_key_2, robot_4203_11.robot_task[task_index][4]);
				if task_char.character_id_1 == 0 then
				   SetAiState(char_type, char_key_1, char_key_2, 1);
		    	   return;
		    	end;
			...
			elseif  robot_4203_11.robot_task[task_index][1] == 4 then
		    	...
		    	CSleep(char_type, char_key_1, char_key_2, robot_4203_11.robot_task[task_index][6]);
			    SetAiState(char_type, char_key_1, char_key_2, 1);
			    return;
			elseif robot_4203_11.robot_task[task_index][1] == 5 then
				local task_char = GetUnityNPC(char_type, char_key_1, char_key_2, robot_4203_11.robot_task[task_index][4]);
	...
	ReAollocTask = function(char_type, char_key_1, char_key_2)
		local task_index = GetTaskIndex(char_type, char_key_1, char_key_2);
	    local dup_process = GetDupProcess(char_type, char_key_1, char_key_2);

	    if dup_process >= 5  then
	       task_index = 10;
	       robot_4203_11.SetTask(char_type, char_key_1, char_key_2, task_index); 
		else
	    ...
	    if task_index <= 0 then
		   ...
		   robot_4203_11.SetTask(char_type, char_key_1, char_key_2, task_index); 
	    		...
	    		--是否完成任务
		local if_finish = robot_4203_11.CheckTaskFinished(char_type, char_key_1, char_key_2);
	...
	SetTask = function(char_type, char_key_1, char_key_2, task_index)
		SetTaskIndex(char_type, char_key_1, char_key_2, task_index);
	        if robot_4203_11.robot_task[task_index][1] == 0 then
		       CSleep(char_type, char_key_1, char_key_2, robot_4203_11.robot_task[task_index][4]);
		       return;
		    end;

		SetRobotTask(char_type, char_key_1, char_key_2,
		robot_4203_11.robot_task[task_index][1], robot_4203_11.robot_task[task_index][4], 
	    robot_4203_11.robot_task[task_index][5], robot_4203_11.robot_task[task_index][6]);
	    SetRobotWalkTarget(char_type, char_key_1, char_key_2);  
	end;
};
