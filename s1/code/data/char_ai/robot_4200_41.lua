robot_4200_41=
{
	...
	init=function(char_type, char_key_1, char_key_2, jingjie, lvl)
		...
		SetPos(char_type, char_key_1, char_key_2, robot_4200_41.born_x, robot_4200_41.born_y, 0);
		SetRealmLvl(char_type, char_key_1, char_key_2, jingjie);
		SetLvl(char_type, char_key_1, char_key_2, lvl);

		SetPre(char_type, char_key_1, char_key_2, 41);
	...
	timeout=function(char_type, char_key_1, char_key_2, time_sec, time_m_sec)
	    ...
	    if task_index <= 0 then
		   ...
		   if robot_4200_41.robot_task[task_index][1] == 0 then
		      CSleep(char_type, char_key_1, char_key_2, robot_4200_41.robot_task[task_index][4]);
		      return;
		   end;

		   SetRobotTask(char_type, char_key_1, char_key_2,
		    robot_4200_41.robot_task[task_index][1], robot_4200_41.robot_task[task_index][4], 
		   	robot_4200_41.robot_task[task_index][5], robot_4200_41.robot_task[task_index][6]);
		   SetRobotWalkTarget(char_type, char_key_1, char_key_2);  
	    end;
		
		--是否完成任务
		local if_finish = robot_4200_41.CheckTaskFinished(char_type, char_key_1, char_key_2);
		...
		if ai_state == 0 then
			robot_4200_41.ProcessBorn(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 1 then
			robot_4200_41.ProcessStand(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 2 then
			robot_4200_41.ProcessPatrol(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 3 then
			robot_4200_41.ProcessWalk(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 4 then
			robot_4200_41.ProcessRunAway(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 5 then
			robot_4200_41.ProcessGoBack(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 6 then
			robot_4200_41.ProcessFighting(char_type, char_key_1, char_key_2, ai_state);
		elseif ai_state == 7 then
			robot_4200_41.ProcessFlow(char_type, char_key_1, char_key_2, ai_state);
		end;
	end;
	...
	CheckTaskFinished=function(char_type, char_key_1, char_key_2)
	    --判定任务类型, 做相应的处理
			...
			if robot_4200_41.robot_task[task_index][1] == 0 then
				...
				--休息交给stand状态去处理
			elseif robot_4200_41.robot_task[task_index][1] == 1 then
		    	--检测是否已经到目标点
		    	if CheckDistance(char_type, char_key_1, char_key_2, robot_4200_41.robot_task[task_index][4], robot_4200_41.robot_task[task_index][6], 20) == 1 then
		    		return true;
		    	end;
			elseif 	robot_4200_41.robot_task[task_index][1] == 2 then
				--检测当前任务需要的type数量是否达标
				local kill_npc_num = GetTaskNpcKillNum(char_type, char_key_1, char_key_2);
				if kill_npc_num >= robot_4200_41.robot_task[task_index][5] then
					return true;
				elseif GetNearestTypeNPC(char_type, char_key_1, char_key_2, robot_4200_41.robot_task[task_index][4]).character_id_1 == 0 then
					return true;
				end;	  
			elseif  robot_4200_41.robot_task[task_index][1] == 3 then
				--检测目标血量是否达标
				local tar_hp_percent = GetTaskNpcHpPercent(char_type, char_key_1, char_key_2);
				if tar_hp_percent <= robot_4200_41.robot_task[task_index][5] then
					return true;	
			    elseif GetUnityNPC(char_type, char_key_1, char_key_2, robot_4200_41.robot_task[task_index][4]).character_id_1 == 0 then
					return true;
				end;
			elseif  robot_4200_41.robot_task[task_index][1] == 4 then
				local collect_npc_num = GetTaskNpcNum(char_type, char_key_1, char_key_2);
				if collect_npc_num >= robot_4200_41.robot_task[task_index][5] then
					return true;
				elseif GetNearestTypeNPC(char_type, char_key_1, char_key_2, robot_4200_41.robot_task[task_index][4]).character_id_1 == 0 then
					return true;
				end; 
			elseif robot_4200_41.robot_task[task_index][1] == 5 then
				local collect_npc_num = GetTaskNpcNum(char_type, char_key_1, char_key_2);
				if collect_npc_num >= robot_4200_41.robot_task[task_index][5] then
					return true;
				elseif GetUnityNPC(char_type, char_key_1, char_key_2, robot_4200_41.robot_task[task_index][4]).character_id_1 == 0 then
	...
	ProcessWalk=function(char_type, char_key_1, char_key_2, cur_state)
		...
		if hate_char.character_id_1 ~= 0 and robot_4200_41.robot_task[task_index][2] >= 1 then
		   --你要战，便作战
		...
		if robot_4200_41.robot_task[task_index][2] == 2 then
		   --我就是好战
		...
		if CheckDistance(char_type, char_key_1, char_key_2, tar_pos.pos_x, tar_pos.pos_y, check_distance) == 1 then
			if robot_4200_41.robot_task[task_index][1] == 1 then
		    	--进入stand状态
		    	SetAiState(char_type, char_key_1, char_key_2, 1);
		    	return;
			elseif 	robot_4200_41.robot_task[task_index][1] == 2 then
				--找一个当前任务需要的type_npc
				local task_char = GetNearestTypeNPC(char_type, char_key_1, char_key_2, robot_4200_41.robot_task[task_index][4]);
			...
			elseif  robot_4200_41.robot_task[task_index][1] == 3 then
				--检测目标血量是否达标
				local task_char = GetUnityNPC(char_type, char_key_1, char_key_2, robot_4200_41.robot_task[task_index][4]);
				if task_char.character_id_1 == 0 then
				   SetAiState(char_type, char_key_1, char_key_2, 1);
		    	   return;
		    	end;
			...
			elseif  robot_4200_41.robot_task[task_index][1] == 4 then
		    	...
		    	CSleep(char_type, char_key_1, char_key_2, robot_4200_41.robot_task[task_index][6]);
			    SetAiState(char_type, char_key_1, char_key_2, 1);
			    return;
			elseif robot_4200_41.robot_task[task_index][1] == 5 then
				local task_char = GetUnityNPC(char_type, char_key_1, char_key_2, robot_4200_41.robot_task[task_index][4]);
	...
	ProcessFighting=function(char_type, char_key_1, char_key_2, cur_state)
		...
		if CastSkillCheck(char_type, char_key_1, char_key_2, 4031, 1) == 0 then
		...
		if CastSkillCheck(char_type, char_key_1, char_key_2, 4011, 1) == 0 then
			CastSkill(char_type, char_key_1, char_key_2,
			tar_char.char_type, tar_char.character_id_1, tar_char.character_id_2,
			4011, 1);
			return;
